...
int ABPEmpty (PtABPNode proot){
	...
	if(Node == NULL){ Error = OK; return 1; }
	else return 0;
}

int ABPElement (PtABPNode pnode){
	PtABPNode Node = pnode;

	if(Node == NULL){ Error = NULL_PTR; return 0;}
...
PtABPNode ABPMaxRep (PtABPNode proot){
	if (ABPEmpty(proot)){ Error = ABP_EMPTY; return NULL; }
...
int ABPTotalSum (PtABPNode proot){
	int sum = 0; PtABPNode Node = proot; PtQueue Queue;

	if (ABPEmpty(proot)){ 	/* arvore vazia */
		Error = ABP_EMPTY; 
		return 0;
	}
	...
	while (!QueueIsEmpty (Queue)){
		...
		sum += Node->Elem;	/* imprimir o elemento */
...
unsigned int ABPMultCount (PtABPNode proot, int pvalue){
	...
	PtABPNode aux; PtStack f;

	if(proot == NULL){ Error = ABP_EMPTY; return 0; }

  	if((f=StackCreate(sizeof(PtABPNode))) == NULL) { Error = NO_MEM; return 0; }

  	StackPush(f, (PtABPNode)&proot);

  	while(!StackIsEmpty(f)){
	    StackPop(f, (PtABPNode)&aux);
	    if(aux->Elem % pvalue ==0) countMul++;
	    if(aux->PtRight) StackPush(f,(PtABPNode)&aux->PtRight);
	    if(aux->PtLeft) StackPush(f,(PtABPNode)&aux->PtLeft);
  	}
...
static int addImpar = 0;
unsigned int ABPOddCount (PtABPNode proot){
	
	if(ABPEmpty(proot)){ Error = ABP_EMPTY; return 0; }

	if((proot->Elem % 2) != 0) addImpar++; 

	if(proot->PtLeft != NULL) ABPOddCount(proot->PtLeft);

	if(proot->PtRight != NULL) ABPOddCount(proot->PtRight);
...
int ABPEvenSum (PtABPNode proot){
	...
	if((proot->Elem % 2) == 0) sumPar += proot->Elem; 

	if(proot->PtLeft != NULL) ABPEvenSum(proot->PtLeft);

	if(proot->PtRight != NULL) ABPEvenSum(proot->PtRight);

	return sumPar;
}

int ABPOddOrderSum (PtABPNode proot){
	PtABPNode p=proot; PtStack stack; int index=1,sum=0;

  	if(proot==NULL) return 0;
  	if((stack=StackCreate(sizeof(PtABPNode*)))==NULL) return 0;

  	StackPush(stack,(PtABPNode)&p);
  	while(!StackIsEmpty(stack)){
	    if(p!=NULL){
	      	p=p->PtLeft;
	      	StackPush(stack,(PtABPNode)&p);
	    	...
	    	StackPop(stack,(PtABPNode)&p);
	      	if(!StackIsEmpty(stack)){
	      		StackPop(stack,(PtABPNode)&p);
				if(index%2!=0) sum+=p->Elem;
				index++;
				p=p->PtRight;
				StackPush(stack,(PtABPNode)&p);
      		}
